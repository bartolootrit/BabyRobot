...
class GridLevel:
    ...
    def get_available_actions(self, x, y, policy=None):
        ...
        if self.maze is not None:
            ...
            if self.fill_center == True:
                if ((x >= 1 and x <= self.width - 2) and (y >= 1 and y <= self.height - 2)):
                    ...
                    if ((x >= 1 and x <= self.width - 2) and (y == 0)):
                        del actions['S']
                    elif ((x >= 1 and x <= self.width - 2) and (y == self.height - 1)):
                        del actions['N']
                    elif ((y >= 1 and y <= self.height - 2) and (x == 0)):
                        del actions['E']
                    elif ((y >= 1 and y <= self.height - 2) and (x == self.width - 1)):
                        del actions['W']
        ...
        if policy is not None:
            ...
            for direction, v in actions.items():
                if v == True:
                    if (direction == 'N') and not (dir_value & Direction.North): actions[
                        'N'] = False
                    if (direction == 'S') and not (dir_value & Direction.South): actions[
                        'S'] = False
                    if (direction == 'E') and not (dir_value & Direction.East):  actions[
                        'E'] = False
                    if (direction == 'W') and not (dir_value & Direction.West):  actions[
                        'W'] = False
    ...
    def draw_level(self, add_maze):
        ...
        with hold_canvas(canvas):
            ...
            if self.fill_center: self.draw_non_grid_area(canvas, [1, 1], self.width - 2,
                                                         self.height - 2)
    ...
    def draw_splash(self, canvas, x, y, puddle_type):
        ...
        if puddle_type > 0:
            ...
            canvas.draw_image(splash_canvas, x_px, y_px, width=self.cell_pixels,
                              height=self.cell_pixels)

    def draw_MDP(self):
        ''' draw the level as an MDP '''

        canvas = RoughCanvas(width=self.width * 150 + 100, height=self.height * 150 + 100,
    ...
    def show_cell_text(self, row, col, value, color='#002', back_color=None):
        ...
        if (self.fill_center == False
            or not (
                (row >= 1 and row <= self.height - 2) and (col >= 1 and col <= self.width - 2))):
    ...
    def get_direction_list_value(self, direction_list):
        ...
        for direction, v in direction_list.items():
            ...
            if v == True:
                ...
                # calculate the postion of the next state
                if direction == 'N':
                    dir_value += Direction.North
                elif direction == 'S':
                    dir_value += Direction.South
                elif direction == 'E':
                    dir_value += Direction.East
                elif direction == 'W':
                    dir_value += Direction.West
